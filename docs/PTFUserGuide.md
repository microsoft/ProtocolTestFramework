...
PTF provides an optional configuration property named `PTF.LogProfileParserPatch.Enabled`, which controls how log file names are generated by file-based log receivers. This property is defined in the `<Common>` configuration group and is intended for use in multi-threaded execution scenarios.

By default, this property is not enabled. When the property is not specified or is set to false, PTF preserves the existing file naming behavior for file log sinks, including any explicitly configured file names. This default behavior is recommended when multi-threading is not enabled and stable, deterministic log file names are required.

When `PTF.LogProfileParserPatch.Enabled` is set to true, PTF generates a unique file name for each file log sink, even if a file name is explicitly specified in the configuration. This behavior is designed to prevent file name conflicts during concurrent test execution. Users should enable this property only when running tests in multi-threaded mode. Enabling it in single-threaded scenarios may result in unnecessary changes to log file naming behavior.

Example:

```
<Group name="Common">
    <Property name="PTF.LogProfileParserPatch.Enabled" value="false"/>
</Group>
```
...
#### File Name Generation Behavior

When `PTF.LogProfileParserPatch.Enabled` is set to true, PTF always generates a unique file name for file log sinks using the following format:

```
<TimeStamp>_<TestAssemblyName>_<Guid>_<ConfiguredFileName>
```

For example:

```
2026-01-22 03_43_15_966_MS-SMB2Model_ServerTestSuite_90135e83_FileServer_Log
```

This optional behavior is intended to prevent file name collisions in scenarios such as:

Parallel or concurrent test execution

Note: 

When this option is enabled, consumers of log files should not rely on a fixed file name and should instead discover log files dynamically (for example, by directory enumeration or pattern matching).
